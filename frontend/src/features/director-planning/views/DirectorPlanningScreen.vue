<template>
  <AppShell fullWidth showLogout @logout="logout">
    <DesktopWorkbenchShell class="director-planning-shell" compact>
      <template #rail>
        <section class="panel director-planning-hero">
          <div class="inline-between">
            <div>
              <p class="eyebrow">Director Planning</p>
              <h2>导演 · 计划面板</h2>
              <p class="muted">{{ project?.name || '加载中...' }} · 分集作用域与预检查</p>
            </div>
            <div class="actions">
              <button @click="goDirectorHome">导演首页</button>
              <button @click="goExecution">执行面板</button>
        ...
        <section class="panel">
          ...
          <div class="actions">
            ...
            <button class="primary" @click="goStoryboard">分镜工作台</button>
            <button @click="goReview">审核工作台</button>
      ...
      <section class="panel">
        ...
        <div class="list compact-list">
          <article class="card" v-for="item in episodes" :key="`ep-check-${item.episode.id}`">
            <label class="check-inline">
...
const restoreFromQuery = (): void => {
  const query = toSingleQuery(route.query);
  const scopeEpisodeId = query.scopeEpisodeId || '';
  const checked = query.checkedEpisodeIds ? query.checkedEpisodeIds.split(',').filter(Boolean) : [];
  selectedEpisodeId.value = scopeEpisodeId;
  checkedEpisodeIds.value = checked;
  ...
  if (scopeEpisodeId || checked.length > 0 || precheckAt.value || precheckSummary.value) {
...
const syncQuery = async (): Promise<void> => {
  ...
  const payload: Record<string, string | undefined> = {
    scopeEpisodeId: selectedEpisodeId.value || undefined,
...
const exportPrecheckCsv = (): void => {
  ...
  link.href = url;
  link.download = `director-precheck-${projectId.value}.csv`;
  link.click();
...
watch(
  () => [selectedEpisodeId.value, checkedEpisodeIds.value.join(','), precheckAt.value],
  () => {
    void syncQuery();
  }
);
